...
void sort_csr(problem_t<i_t, f_t>& problem)
{
  raft::common::nvtx::range fun_scope("sort_csr");
  auto stream_view = problem.handle_ptr->get_stream();
  rmm::device_uvector<std::byte> d_tmp_storage_bytes(0, stream_view);
  size_t tmp_storage_bytes{0};
  auto num_segments = problem.n_constraints;
  auto num_items    = problem.nnz;
