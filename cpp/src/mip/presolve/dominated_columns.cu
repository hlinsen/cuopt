  ...
  bound_presolve_t<i_t, f_t>& bounds_presolve)
{
  ...
  for (int i = 0; i < problem.n_variables; ++i) {
    auto original_var_idx = variable_mapping[i];
    // Skip free vars
  ...
  typename problem_t<i_t, f_t>::host_view_t& host_problem, std::vector<i_t> const& candidates)
{
  ...
  for (auto col : candidates) {
    ...
    for (int j = 0; j < nnz_in_col; ++j) {
      ...
      if (host_problem.row_types[row] == row_type_t::RANGED_OR_EQUALITY ||
          host_problem.row_types[row] == row_type_t::GE_INEQUALITY ||
          host_problem.row_types[row] == row_type_t::LE_INEQUALITY) {
  ...
  domination_order_t xk_order)
{
  ...
  // std::cout << "Signature " << xj << " is " << signatures[xj].first << ", " <<
  // signatures[xj].second
  //           << std::endl;
  // std::cout << "Signature " << xk << " is " << signatures[xk].first << ", " <<
  // signatures[xk].second
  //           << std::endl;
  ...
  // auto variable_names     = problem.original_problem_ptr->get_variable_names();
  // std::cout << "checking domination " << h_variable_mapping[xj] << "("
  //           << variable_names[h_variable_mapping[xj]] << ") -> " << h_variable_mapping[xk] << "("
  ...
  // std::cout << "Signature " << h_variable_mapping[xj] << "("
  //           << variable_names[h_variable_mapping[xj]] << ") and " << h_variable_mapping[xk] <<
  //           "("
  //           << variable_names[h_variable_mapping[xk]] << ") is true" << std::endl;
  ...
  if (cj > ck + COEFF_EPSILON) { return false; }
  ...
  for (int i = 0; i < xj_nnz; ++i) {
    ...
    if (host_problem.row_types[row_xj] == row_type_t::GE_INEQUALITY) {
      coeff_xk = -coeff_xk;
      coeff_xj = -coeff_xj;
    }

    // std::cout << "row_lb: " << row_lb << ", row_ub: " << row_ub << std::endl;
    // std::cout << "is_ranged_or_equality: "
    //           << static_cast<int>(host_problem.row_types[row_xj] ==
    //           row_type_t::RANGED_OR_EQUALITY)
    //           << std::endl;
    // std::cout << "row: " << row_xj << ", coeff1: " << coeff_xj << ", coeff2: " << coeff_xk
    //           << std::endl;

    if (host_problem.row_types[row_xj] == row_type_t::RANGED_OR_EQUALITY) {
  ...
  for (int i = 0; i < xk_nnz; ++i) {
    ...
    if (host_problem.row_types[row_xk] == row_type_t::GE_INEQUALITY) {
      coeff_xk = -coeff_xk;
      coeff_xj = -coeff_xj;
    }
    ...
    if (host_problem.row_types[row_xk] == row_type_t::RANGED_OR_EQUALITY) {
      // For equality constraints, coefficients must be equal (within epsilon)
  ...
  domination_order_t xk_order)
{
  ...
  //           << ", xj_is_ub_implied: " << xj_is_ub_implied << std::endl;
  cuopt_assert(var_fixed, "Variable is not fixed");
}

template <typename i_t, typename f_t>
bool dominated_columns_t<i_t, f_t>::skip_domination(
  typename problem_t<i_t, f_t>::host_view_t& host_problem,
  i_t xj,
  i_t xk,
  domination_order_t xj_order,
...
{
  auto h_variable_mapping = cuopt::host_copy(problem.presolve_data.variable_mapping, stream);
  f_t original_xk_lb      = host_problem.original_variable_lower_bounds[h_variable_mapping[xk]];
  f_t original_xk_ub      = host_problem.original_variable_upper_bounds[h_variable_mapping[xk]];
  if (xj_order == domination_order_t::REGULAR && xk_order == domination_order_t::REGULAR &&
      original_xk_lb == -std::numeric_limits<f_t>::infinity()) {
    return true;
  }
  if (xj_order == domination_order_t::REGULAR && xk_order == domination_order_t::NEGATED_XK &&
      original_xk_ub == std::numeric_limits<f_t>::infinity()) {
    return true;
  }
  if (xj_order == domination_order_t::NEGATED_XJ && xk_order == domination_order_t::REGULAR &&
      original_xk_lb == -std::numeric_limits<f_t>::infinity()) {
    return true;
  }
  if (xj_order == domination_order_t::NEGATED_XJ && xk_order == domination_order_t::NEGATED_XK &&
      original_xk_ub == std::numeric_limits<f_t>::infinity()) {
    return true;
  }
  return false;
...
void dominated_columns_t<i_t, f_t>::presolve(bound_presolve_t<i_t, f_t>& bounds_presolve)
{
  ...
  for (const auto& [xj, pair] : shortest_rows) {
    ...
    for (int j = 0; j < nnz_in_row; ++j) {
      ...
      auto original_var_idx = h_variable_mapping[xk];
      // Skip free vars
      if (original_var_idx >= problem.original_problem_ptr->get_n_variables()) { continue; }
      if (xj == xk) { continue; }
      ...
      for (auto xk_order : {domination_order_t::REGULAR, domination_order_t::NEGATED_XK}) {
        // if (skip_domination(host_problem, xj, xk, xj_order, xk_order)) { continue; }
  ...
  cuopt_assert(!has_free_vars, "Free variables are not allowed");
  std::cout << "Number of dominated variables after checking free vars: " << num_dominated_vars
            << std::endl;
  // exit(1);
