...
inline char feasible_solution_symbol(search_strategy_t strategy, bool show_diving)
{
  ...
  if (strategy == search_strategy_t::RINS || strategy == search_strategy_t::RENS ||
      strategy == search_strategy_t::DINS)
    return 'S';
  ...
  switch (strategy) {
    ...
    case search_strategy_t::DINS: return 'S';
  }
                                                            ...
                                                            double work_units)
{
  ...
  if (settings_.solution_callback != nullptr) { settings_.solution_callback(user_assignment, obj); }
...
bool branch_and_bound_t<i_t, f_t>::dive_with(diving_worker_t<i_t, f_t>* worker, i_t backtrack_limit)
{
  ...
  if (worker->search_strategy != search_strategy_t::RINS &&
      ...
      worker->search_strategy != search_strategy_t::DINS) {
...
bool branch_and_bound_t<i_t, f_t>::launch_submip_worker(const std::vector<f_t>& sol)
{
  ...
  const bool rens_enabled = settings_.submip_settings.rens != 0;
  const bool dins_enabled = settings_.submip_settings.dins != 0;
  if (!rins_enabled && !rens_enabled && !dins_enabled) { return false; }
  if (submip_worker_pool_.num_idle() == 0) { return false; }

  std::vector<f_t> current_incumbent;
  mutex_upper_.lock();
  const bool has_incumbent = incumbent_.has_incumbent;
  if (has_incumbent) { current_incumbent = incumbent_.x; }
  mutex_upper_.unlock();
  ...
  const bool rins_eligible = has_incumbent && rins_enabled;
  const bool dins_eligible =
    has_incumbent && dins_enabled && dins_schedule_.should_launch(true, nodes_explored);
  const submip_bandit_t::eligibility_t eligible{rins_eligible, dins_eligible};

  std::optional<submip_heuristic_t> selected;
  bool use_rens = false;
  if (rins_eligible || dins_eligible) {
    selected = submip_bandit_.select(eligible);
    if (!selected.has_value()) {
      mutex_submip_bandit_.unlock();
      return false;
    }
  } else if (rens_enabled) {
    use_rens = true;
    current_incumbent.clear();
  } else {
    mutex_submip_bandit_.unlock();
    return false;
  }
  ...
  if (worker == nullptr) {
    if (selected.has_value()) { submip_bandit_.cancel(*selected); }
    mutex_submip_bandit_.unlock();
    return false;
  }
  ...
  worker->search_strategy =
    use_rens
      ...
      : (*selected == submip_heuristic_t::DINS ? search_strategy_t::DINS
                                                : search_strategy_t::RINS);
  worker->set_active();
...
#pragma omp task priority(CUOPT_DEFAULT_TASK_PRIORITY) affinity(worker) \
  ...
  if (settings_.inside_submip) {
    run_submip_worker(worker, sol, current_incumbent, heuristic);
  } else {
#pragma omp task priority(CUOPT_DEFAULT_TASK_PRIORITY) affinity(worker) \
  firstprivate(worker, sol, current_incumbent, heuristic)
    run_submip_worker(worker, sol, current_incumbent, heuristic);
  }
...
template <typename i_t, typename f_t>
void branch_and_bound_t<i_t, f_t>::run_submip_worker(
  ...
  submip_heuristic_t heuristic)
{
  ...
  const bool improved     = heuristic == submip_heuristic_t::DINS
                              ? dins(worker, node_solution)
                              : recursive_submip(
                                  worker, current_incumbent, var_types_, false, false);
...
template <typename i_t, typename f_t>
typename branch_and_bound_t<i_t, f_t>::submip_result_t branch_and_bound_t<i_t, f_t>::solve_submip(
  diving_worker_t<i_t, f_t>* worker,
  const std::vector<f_t>& current_incumbent,
  const std::vector<variable_type_t>& var_types,
  submip_stats_t& submip_stats,
  ...
  f_t local_branching_rhs)
{
  ...
  if (submip_settings.branch_and_bound_simplex_iteration_limit <= 0) {
    return {mip_status_t::ITERATION_LIMIT, false};
  }

  submip_settings.time_limit = settings_.time_limit - toc(exploration_stats_.start_time);
  if (submip_settings.time_limit <= 0) { return {mip_status_t::TIME_LIMIT, false}; }
  ...
  submip_settings.submip_settings.dins = settings_.submip_settings.dins != 0 && !max_recursion;
  ...
  if (!local_branching_variables.empty()) {
    dins_neighborhood_t<i_t, f_t> neighborhood;
    neighborhood.soft_variables    = local_branching_variables;
    neighborhood.soft_coefficients = local_branching_coefficients;
    neighborhood.soft_rhs          = local_branching_rhs;
    append_local_branching_constraint(submip_problem, neighborhood);
  }
  ...
  if (presolver_status == third_party_presolve_status_t::INFEASIBLE ||
    ...
    return {mip_status_t::INFEASIBLE, false};
  }
  ...
  if (submip_problem.num_rows == 0 || submip_problem.num_cols == 0) {
    ...
    return {mip_status_t::OPTIMAL, false};
  }

  if (toc(exploration_stats_.start_time) > settings_.time_limit) {
    solver_status_ = mip_status_t::TIME_LIMIT;
    return {mip_status_t::TIME_LIMIT, false};
  }
  ...
  submip_settings.solution_callback =
    [this, &presolver, fixrate, &submip_stats, &found_improvement, log_prefix, worker](
      const std::vector<f_t>& solution, f_t) {
      if (this->set_solution_from_submip(
            worker->leaf_problem, solution, presolver, submip_stats, fixrate, log_prefix)) {
        found_improvement.store(true);
      }
    };
  ...
  if (submip_status == mip_status_t::NUMERICAL) {
    return {submip_status, found_improvement.load()};
  }
...
template <typename i_t, typename f_t>
bool branch_and_bound_t<i_t, f_t>::dins(diving_worker_t<i_t, f_t>* worker,
                                        const std::vector<f_t>& node_solution)
{
  raft::common::nvtx::range scope("BB::dins_thread");
  if (worker->orbital_fixing) { worker->orbital_fixing->disable(); }

  const i_t submip_level       = settings_.submip_settings.level + 1;
  const std::string log_prefix = std::format("[DINS {}] ", submip_level);
  ++dins_stats_.total_calls;

  auto snapshot_incumbent = [this]() {
    std::vector<f_t> current_incumbent;
    std::vector<bool> changed_incumbent;
    mutex_upper_.lock();
    current_incumbent = incumbent_.x;
    if (dins_previous_incumbent_.empty()) {
      dins_changed_incumbent_.assign(current_incumbent.size(), false);
    } else {
      assert(dins_previous_incumbent_.size() == current_incumbent.size());
      for (i_t j = 0; j < static_cast<i_t>(current_incumbent.size()); ++j) {
        if (std::abs(current_incumbent[j] - dins_previous_incumbent_[j]) >
            settings_.integer_tol) {
          dins_changed_incumbent_[j] = true;
        }
      }
    }
    dins_previous_incumbent_ = current_incumbent;
    changed_incumbent        = dins_changed_incumbent_;
    mutex_upper_.unlock();
    return std::make_pair(std::move(current_incumbent), std::move(changed_incumbent));
  };

  auto [current_incumbent, changed_incumbent] = snapshot_incumbent();
  ...
  while (keep_searching && solver_status_ == mip_status_t::UNSET && is_running_ &&
    ...
    auto neighborhood = build_dins_neighborhood<i_t, f_t>(node_solution,
                                                          root_relax_soln_.x,
                                                          current_incumbent,
                                                          changed_incumbent,
                                                          var_types_,
                                                          settings_.fixed_tol,
                                                          settings_.integer_tol,
                                                          search.current_radius,
                                                          worker->leaf_problem.lower,
                                                          worker->leaf_problem.upper,
                                                          worker->bounds_changed);

    const bool has_soft_variables = !neighborhood.soft_variables.empty();
    const bool use_local_branching =
      neighborhood.soft_variables.size() > static_cast<std::size_t>(search.current_radius);
    const i_t num_restricted = neighborhood.num_hard_fixed + neighborhood.num_rebounded;
    const f_t fixrate        = static_cast<f_t>(num_restricted) / num_integers;

    DEBUG_SUBMIP("{}hard-fixed={}, rebound={}, soft={}, radius={}",
                 log_prefix,
    ...
    const bool feasible = worker->node_presolver.bounds_strengthening(
      settings_, worker->bounds_changed, worker->leaf_problem.lower, worker->leaf_problem.upper);

    submip_result_t result;
    if (!feasible) {
      dins_stats_.save_infeasible(fixrate);
      result = {mip_status_t::INFEASIBLE, false};
    } else if (use_local_branching) {
      result = solve_submip(worker,
                            current_incumbent,
                            var_types_,
                            dins_stats_,
                            fixrate,
      ...
      result =
        solve_submip(worker, current_incumbent, var_types_, dins_stats_, fixrate, 0, false);
    }
    ...
    if (result.improved) {
      ...
      changed_incumbent       = std::move(incumbent_snapshot.second);
    }
    keep_searching = search.advance(
      result.improved, result.status == mip_status_t::NODE_LIMIT, has_soft_variables);
  }

  DEBUG_SUBMIP("{}success={}, infeasible={}, empty={}, calls={}",
               log_prefix,
               dins_stats_.total_success.load(),
               dins_stats_.total_infeasible.load(),
               dins_stats_.total_empty.load(),
               dins_stats_.total_calls.load());
  return improved;
}

template <typename i_t, typename f_t>
bool branch_and_bound_t<i_t, f_t>::recursive_submip(
  diving_worker_t<i_t, f_t>* worker,
  const std::vector<f_t>& current_incumbent,
  const std::vector<variable_type_t>& var_types,
  bool is_root_heuristic,
  bool return_worker_to_pool)
{
  ...
  while (solver_status_ == mip_status_t::UNSET && is_running_ && !worker->halt) {
    ...
    if (num_frac == 0) {
      ...
      improved =
        add_feasible_solution(leaf_obj, current_sol, -1, worker->search_strategy) || improved;
      ...
      if (is_feasible) {
        ...
        if (!is_root_heuristic) {
          DEBUG_SUBMIP("{}Running a quick DFS. fixrate={:.4g} ({}/{})",
                       ...
                       num_integers);
          improved =
            dive_with(worker, settings_.submip_settings.dfs_max_backtrack) || improved;
        }
      }

    } else {
      const submip_result_t result = solve_submip(worker,
                                                  current_incumbent,
                                                  var_types,
                                                  submip_stats,
                                                  fixrate,
...
#pragma omp taskgroup
  ...
  DEBUG_SUBMIP("DINS: success={}, infeasible={}, empty={}, calls={}",
               dins_stats_.total_success.load(),
               dins_stats_.total_infeasible.load(),
               dins_stats_.total_empty.load(),
               dins_stats_.total_calls.load());
