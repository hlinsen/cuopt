  ...
  submip_heuristic_t heuristic)
{
  ...
  const bool improved =
    heuristic == submip_heuristic_t::DINS
      ? dins(worker, node_solution)
      : recursive_submip(worker, current_incumbent, var_types_, settings_, false);
  ...
  while (keep_searching && solver_status_ == mip_status_t::UNSET && is_running_ &&
    ...
    if (!feasible) {
      ...
      if (use_local_branching) {
        ...
        result = solve_submip(
          worker, current_incumbent, var_types_, dins_stats_, fixrate, 0, submip_settings);
      }
    }
